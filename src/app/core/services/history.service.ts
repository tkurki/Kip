...
export class HistoryService {
  ...
  // Track whether the history API is available on the server
  private historyApiAvailable$ = new BehaviorSubject<boolean | null>(null);
  private historyApiUrl = `${window.location.protocol}//${window.location.host}/signalk/v2/api/history`;

  /**
   * Check if the history API is available on the server
   * This method checks for the existence of the /signalk/v2/api/history endpoint
   * and caches the result to avoid repeated checks.
   */
  public checkHistoryApiAvailability(): Observable<boolean> {
    // If we've already checked, return the cached value
    const currentValue = this.historyApiAvailable$.getValue();
    if (currentValue !== null) {
      return of(currentValue);
    }

    // Try to fetch from /paths endpoint (lightest endpoint)
    const url = `${this.historyApiUrl}/paths`;
    
    return this.http.get(url, { observe: 'response' }).pipe(
      map(response => {
        const available = response.status === 200;
        this.historyApiAvailable$.next(available);
        if (available) {
          console.log('[History Service] History API detected and available');
        }
        return available;
      }),
      catchError(error => {
        console.log('[History Service] History API not available:', error.status || 'unknown error');
        this.historyApiAvailable$.next(false);
        return of(false);
      })
    );
  }
  ...
  /**
   * Get the current state of history API availability
   */
  public isHistoryApiAvailable(): Observable<boolean | null> {
    return this.historyApiAvailable$.asObservable();
  }
  ...
  public fetchHistoryData(
    ...
    // Use provided URL or default to the service's configured URL
    const baseUrl = historyApiUrl || this.historyApiUrl;
