...
export class DatasetService implements OnDestroy {
  ...
  private start(uuid: string): void {
    ...
    this.fetchHistoryData(configuration, dataSource, angleDomain).then(() => {
  ...
  private async fetchHistoryData(configuration: IDatasetServiceDatasetConfig, dataSource: IDatasetServiceDataSource, angleDomain: AngleDomain): Promise<void> {
    const { path } = configuration;
    try {
      const endTime = new Date().toISOString();
      const historyLengthInMillis = getHistoryLength(configuration)
      console.log(configuration)
      console.log(dataSource.maxDataPoints)
      const resolution = historyLengthInMillis / dataSource.maxDataPoints / 1000; // in seconds
      const startTime = new Date(Date.now() - historyLengthInMillis).toISOString();

      const historyUrl = `${window.location.origin}/signalk/v1/history/values?from=${startTime}&to=${endTime}&resolution=${resolution}&paths=${encodeURIComponent(path.substring('self.'.length))}`;
...
const getHistoryLength = (configuration: IDatasetServiceDatasetConfig) => {
  switch (configuration.timeScaleFormat) {
    ...
    case "Last Minute":
      return 1 * 60 * 1000;
    case "hour":
      return configuration.period * 60 * 60 * 1000;
    case "minute":
      return configuration.period * 60 * 1000;
    case "second":
      return configuration.period * 1000;
    default:
      return 10 * 60 * 1000;
  }
}